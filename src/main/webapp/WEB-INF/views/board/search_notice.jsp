		...
		<div class="banana">
			<div class="board_body">
				...
				<div class="btns">
					<ul class="pagination">
						<c:if test="${page.prev}">
							<li>[<a href="/board/search_notice?keyword=${keyword}&num=${page.startPageNum - 1}">이전</a>]</li>
						</c:if>
						<c:forEach begin="${page.startPageNum}" end="${page.endPageNum}" var="num">
							<li>
								<c:if test="${select != num}">
									<a href="/board/search_notice?keyword=${keyword}&num=${num}">${num}</a>
								</c:if>
								<c:if test="${select == num}"> 
						...
						<c:if test="${page.next}">
							<li>[<a href="/board/search_notice?keyword=${keyword}&num=${page.endPageNum + 1}">다음</a>]</li>
						</c:if>
					</ul>
				</div>
			</div>
